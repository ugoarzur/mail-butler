...
class RuleBasedClassifier(BaseClassifier):
    ...
    def classify(
        ...
        if is_noreply and TRANSACTIONAL_PATTERNS.search(subject):
            return self._result(email_id, EmailCategory.TRANSACTIONAL, 0.9)
        ...
        if any(d in sender_domain for d in TRANSACTIONAL_DOMAINS) and TRANSACTIONAL_PATTERNS.search(
            ...
            return self._result(email_id, EmailCategory.TRANSACTIONAL, 0.85)
        ...
        if any(d in sender_domain for d in SOCIAL_DOMAINS) and SOCIAL_PATTERNS.search(subject):
            return self._result(email_id, EmailCategory.SOCIAL, 0.85)
        ...
        if TRANSACTIONAL_PATTERNS.search(subject):
            return self._result(email_id, EmailCategory.TRANSACTIONAL, 0.6)
    ...
    def _result(self, email_id: str, category: EmailCategory, confidence: float) -> Classification:
